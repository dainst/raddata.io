...
def ls8_vegrat1_single(file):
    data = gdalnumeric.LoadFile((file))
    data = data.astype(numpy.float32)
    atsavi = 1.22 * ((data[4, :, :] - 1.22 * data[3, :, :] - 0.03 * numpy.ones_like(data[0, :, :])) / (
                1.22 * data[4, :, :] + data[3, :, :] - 1.22 * 0.03 * numpy.ones_like(
    ...
    del dvimss
    evi = 2.5 * ((data[4, :, :] - data[3, :, :]) / (
                (data[4, :, :] + 6 * data[3, :, :] - 7.5 * data[1, :, :]) + numpy.ones_like(ndvi)))
    ...
    del gvmi
    gari = (data[4, :, :] - (data[2, :, :] - (data[1, :, :] - data[3, :, :]))) / (
                data[4, :, :] - (data[2, :, :] + (data[1, :, :] - data[3, :, :])))
    ...
    del ivi
    ipvi = ((ndvi + numpy.ones_like(ndvi)) * ((data[4, :, :]))) / (
                ((data[4, :, :] + data[3, :, :])) / (2 * numpy.ones_like(data[4, :, :])))
    ...
    del logratio
    msnirred = ((data[4, :, :] / data[3, :, :]) - numpy.ones_like(data[4, :, :])) / (
        numpy.sqrt((data[4, :, :] / data[3, :, :]) + numpy.ones_like(data[4, :, :])))
    writefile(file, msnirred, "msnirred")
    del msnirred
    mcrig = (1 / data[1, :, :] - 1 / data[2, :, :]) * data[4, :, :]
    writefile(file, mcrig, "mcrig")
    del mcrig
    msavi = (2 * (data[4, :, :]) + numpy.ones_like(data[4, :, :]) - numpy.sqrt(
        (2 * data[4, :, :] + numpy.ones_like(data[3, :, :])) ** 2 - 8 * (data[4, :, :]) - data[3, :, :])) / 2
    ...
    del ndsi
    panndvi = (data[4, :, :] - (data[3, :, :] + data[2, :, :] + data[1, :, :])) / (
    ...
    del s1nir
    sarvi2 = 2.5 * ((data[4, :, :] - data[3, :, :]) / (
                numpy.ones_like(data[1, :, :]) + 6 * data[3, :, :] - 7.5 * data[1, :, :]))
    ...
    del sqrtirr
    sbi = 0.3037 * data[1, :, :] + 0.2793 * data[2, :, :] + 0.4743 * data[3, :, :] + 0.5585 * data[4, :,
                                                                                              :] + 0.5082 * data[5, :,
                                                                                                            :] + 0.1863 * data[
                                                                                                                          6,
                                                                                                                          :,
    ...
    del sbi
    gvi = -0.2848 * data[1, :, :] - 0.2435 * data[2, :, :] - 0.5436 * data[3, :, :] + 0.7243 * data[4, :,
                                                                                               :] + 0.084 * data[5, :,
                                                                                                            :] - 0.18 * data[
                                                                                                                        6,
                                                                                                                        :,
