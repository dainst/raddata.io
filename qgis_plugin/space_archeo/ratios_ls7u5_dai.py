...
def ls7_vegrat1_single(file):
    ...
    del arvi
    arvi2 = -0.18 * numpy.ones_like(band[3, :, :]) + 1.17 * (
    ...
    del dvimss
    evi = 2.5 * ((band[3, :, :] - band[2, :, :]) / (
                (band[3, :, :] + 6 * band[2, :, :] - 7.5 * band[0, :, :]) + numpy.ones_like(ndvi)))
    ...
    del gvmi
    gari = (band[3, :, :] - (band[1, :, :] - (band[0, :, :] - band[2, :, :]))) / (
                band[3, :, :] - (band[1, :, :] + (band[0, :, :] - band[2, :, :])))
    ...
    del ivi
    ipvi = ((ndvi + numpy.ones_like(ndvi)) * ((band[3, :, :]))) / (
                ((band[3, :, :] + band[2, :, :])) / (2 * numpy.ones_like(band[3, :, :])))
    ...
    del logratio
    msnirred = ((band[3, :, :] / band[2, :, :]) - numpy.ones_like(band[3, :, :])) / (
        numpy.sqrt((band[3, :, :] / band[2, :, :]) + numpy.ones_like(band[3, :, :])))
    writefile(file, msnirred, "msnirred")
    del msnirred
    mcrig = (1 / band[0, :, :] - 1 / band[1, :, :]) * band[3, :, :]
    writefile(file, mcrig, "mcrig")
    del mcrig
    msavi = (2 * (band[3, :, :]) + numpy.ones_like(band[3, :, :]) - numpy.sqrt(
        (2 * band[3, :, :] + numpy.ones_like(band[2, :, :])) ** 2 - 8 * (band[3, :, :]) - band[2, :, :])) / 2
    ...
    del ndsi
    panndvi = (band[3, :, :] - (band[2, :, :] + band[1, :, :] + band[0, :, :])) / (
    ...
    del s1nir
    sarvi2 = 2.5 * ((band[3, :, :] - band[2, :, :]) / (
                numpy.ones_like(band[0, :, :]) + 6 * band[2, :, :] - 7.5 * band[0, :, :]))
    ...
    del sqrtirr
    sbi = 0.3037 * band[0, :, :] + 0.2793 * band[1, :, :] + 0.4743 * band[2, :, :] + 0.5585 * band[3, :,
                                                                                              :] + 0.5082 * band[4, :,
                                                                                                            :] + 0.1863 * band[
                                                                                                                          5,
                                                                                                                          :,
