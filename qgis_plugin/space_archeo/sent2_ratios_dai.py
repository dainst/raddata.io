...
def S2_10m_vegrat1_single(file):
    data = gdalnumeric.LoadFile((file))
    data = data.astype(numpy.float32)
    atsavi = 1.22 * ((data[3, :, :] - 1.22 * data[2, :, :] - 0.03 * numpy.ones_like(data[0, :, :])) / (
                1.22 * data[3, :, :] + data[2, :, :] - 1.22 * 0.03 * numpy.ones_like(
            data[0, :, :] + (0.08 + 0.08 * 1.22 ** 2) * numpy.ones_like(data[0, :, :]))))
    writefile(file, atsavi, "atsavi")
    del atsavi
    avi = 2.0 * data[3, :, :] - data[2, :, :]
    writefile(file, avi, "avi")
    del avi
    arvi = ((data[3, :, :] - data[2, :, :]) - (data[2, :, :] - data[0, :, :])) / (
                (data[3, :, :] + data[2, :, :]) - (data[2, :, :] - data[0, :, :]))
    writefile(file, arvi, "arvi")
    del arvi
    arvi2 = -0.18 * numpy.ones_like(data[3, :, :]) + 1.17 * (
    ...
    ctvi = ((ndvi + numpy.ones_like(ndvi)) / numpy.abs(ndvi + numpy.ones_like(ndvi))) * numpy.sqrt(
        numpy.abs(ndvi + 0.5 * numpy.ones_like(ndvi)))
    ...
    del gdvi
    evi = 2.5 * ((data[3, :, :] - data[2, :, :]) / (
                (data[3, :, :] + 6 * data[2, :, :] - 7.5 * data[0, :, :]) + numpy.ones_like(ndvi)))
    ...
    del evi22
    gari = (data[3, :, :] - (data[1, :, :] - (data[0, :, :] - data[2, :, :]))) / (
    ...
    del ivi
    ipvi = ((ndvi + numpy.ones_like(ndvi)) * ((data[3, :, :]))) / (
                ((data[3, :, :] + data[2, :, :])) / (2 * numpy.ones_like(data[3, :, :])))
    ...
    del logratio
    msnirred = ((data[3, :, :] / data[2, :, :]) - numpy.ones_like(data[3, :, :])) / (
        numpy.sqrt((data[3, :, :] / data[2, :, :]) + numpy.ones_like(data[3, :, :])))
    writefile(file, msnirred, "msnirred")
    del msnirred
    mcrig = (1 / data[0, :, :] - 1 / data[1, :, :]) * data[3, :, :]
    writefile(file, mcrig, "mcrig")
    msavi = (2 * (data[3, :, :]) + numpy.ones_like(data[3, :, :]) - numpy.sqrt(
        (2 * data[3, :, :] + numpy.ones_like(data[2, :, :])) ** 2 - 8 * (data[3, :, :]) - data[2, :, :])) / 2
    ...
    del ri
    panndvi = (data[3, :, :] - (data[2, :, :] + data[1, :, :] + data[0, :, :])) / (
    ...
    del rnr
    sarvi2 = 2.5 * ((data[3, :, :] - data[2, :, :]) / (
                numpy.ones_like(data[0, :, :]) + 6 * data[2, :, :] - 7.5 * data[0, :, :]))
...
def S2_20m_vegrat1_single(file):
    data = gdalnumeric.LoadFile((file))
    data = data.astype(numpy.float32)
    atsavi = 1.22 * ((data[6, :, :] - 1.22 * data[2, :, :] - 0.03 * numpy.ones_like(data[0, :, :])) / (
                1.22 * data[6, :, :] + data[2, :, :] - 1.22 * 0.03 * numpy.ones_like(
            data[0, :, :] + (0.08 + 0.08 * 1.22 ** 2) * numpy.ones_like(data[0, :, :]))))
    writefile(file, atsavi, "atsavi")
    del atsavi
    ...
    writefile(file, avi, "avi")
    del avi
    arvi = ((data[6, :, :] - data[2, :, :]) - (data[2, :, :] - data[0, :, :])) / (
                (data[6, :, :] + data[2, :, :]) - (data[2, :, :] - data[0, :, :]))
    writefile(file, arvi, "arvi")
    del arvi
    arvi2 = -0.18 * numpy.ones_like(data[6, :, :]) + 1.17 * (
                (data[6, :, :] - data[2, :, :]) / (data[6, :, :] + data[2, :, :]))
    writefile(file, arvi2, "arvi2")
    del arvi2
    ...
    del dvimss
    evi = 2.5 * ((data[6, :, :] - data[2, :, :]) / (
                (data[6, :, :] + 6 * data[2, :, :] - 7.5 * data[0, :, :]) + numpy.ones_like(ndvi)))
    writefile(file, evi, "evi")
    del evi
    ...
    writefile(file, evi22, "evi22")
    del evi22
    gvmi = ((data[6, :, :] + 0.1 * numpy.ones_like(ndvi)) - (data[7, :, :] + 0.02 * numpy.ones_like(ndvi))) / (
                (data[6, :, :] + 0.1 * numpy.ones_like(ndvi)) + (data[7, :, :] + 0.02 * numpy.ones_like(ndvi)))
    writefile(file, gvmi, "gvmi")
    del gvmi
    gari = (data[6, :, :] - (data[1, :, :] - (data[0, :, :] - data[2, :, :]))) / (
                data[6, :, :] - (data[1, :, :] + (data[0, :, :] - data[2, :, :])))
    writefile(file, gari, "gari")
    del gari
    gli = (2 * data[1, :, :] - data[2, :, :] - data[0, :, :]) / (2 * data[1, :, :] + data[2, :, :] + data[0, :, :])
    writefile(file, gli, "gli")
    ...
    writefile(file, ivi, "ivi")
    del ivi
    ipvi = ((ndvi + numpy.ones_like(ndvi)) * ((data[6, :, :]))) / (
                ((data[6, :, :] + data[2, :, :])) / (2 * numpy.ones_like(data[6, :, :])))
    writefile(file, ipvi, "ipvi")
    del ipvi
    intensity = 1 / 30.5 * (data[0, :, :] + data[1, :, :] + data[2, :, :])
    writefile(file, intensity, "intensity")
    ...
    writefile(file, logratio, "logratio")
    del logratio
    msnirred = ((data[6, :, :] / data[2, :, :]) - numpy.ones_like(data[6, :, :])) / (
        numpy.sqrt((data[6, :, :] / data[2, :, :]) + numpy.ones_like(data[6, :, :])))
    writefile(file, msnirred, "msnirred")
    del msnirred
    mcrig = (1 / data[0, :, :] - 1 / data[1, :, :]) * data[6, :, :]
    writefile(file, mcrig, "mcrig")
    msavi = (2 * (data[6, :, :]) + numpy.ones_like(data[6, :, :]) - numpy.sqrt(
        (2 * data[6, :, :] + numpy.ones_like(data[2, :, :])) ** 2 - 8 * (data[6, :, :]) - data[2, :, :])) / 2
    writefile(file, msavi, 'msavi')
    del msavi
    ...
    del ndsi
    panndvi = (data[6, :, :] - (data[2, :, :] + data[1, :, :] + data[0, :, :])) / (
                data[6, :, :] + (data[2, :, :] + data[1, :, :] + data[0, :, :]))
    writefile(file, panndvi, 'panndvi')
    del panndvi
    ...
    writefile(file, sqrtirr, 'sqrtirr')
    del sqrtirr
    sbi = 0.3037 * data[0, :, :] + 0.2793 * data[1, :, :] + 0.4743 * data[2, :, :] + 0.5585 * data[6, :,
                                                                                              :] + 0.5082 * data[7, :,
                                                                                                            :] + 0.1863 * data[
                                                                                                                          8,
                                                                                                                          :,
    ...
    del sbi
    gvi = -0.2848 * data[0, :, :] - 0.2435 * data[1, :, :] - 0.5436 * data[2, :, :] + 0.7243 * data[6, :,
                                                                                               :] + 0.084 * data[7, :,
                                                                                                            :] - 0.18 * data[
                                                                                                                        8,
                                                                                                                        :,
    ...
    del gvi
    wet = 0.1509 * data[0, :, :] + 0.1973 * data[1, :, :] + 0.3279 * data[2, :, :] + 0.3406 * data[6, :,
                                                                                              :] - 0.7112 * data[7, :,
                                                                                                            :] - 0.4572 * data[
                                                                                                                          8,
                                                                                                                          :,
